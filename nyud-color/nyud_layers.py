...
class NYUDSegDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
   	...
   	self.mean_hha = np.array((194.421, 90.766, 126.557), dtype=np.float32)
        self.mean_d = np.array((106.768), dtype=np.float32)
    ...
    def load_sceneLabel(self,idx):
        allSceneLabel = scipy.io.loadmat('{}/label/labels.mat'.format(self.nyud_dir))['label']
	idx = int(idx) - 5001
        scenelabel =  allSceneLabel[0,idx]
